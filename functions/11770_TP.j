...
function TP takes nothing returns nothing
	...
	loop
		...
		set ScoreArr[wp + uP]=(Re[wp + uP]- 1)* 30 + vv[wp + uP]* 20 + R2I(lv)* 7 - ev[wp + uP]* 15 + Qv[wp + uP]* 25
		set up[wp]= false
		set wp = wp + 1
	endloop
	set GB = 1
	loop
		...
		loop
			exitwhen wp > Up
			if wN < ScoreArr[wp + uP]and up[wp]==false then
				set wN = ScoreArr[wp + uP]
				set lv =(100. *(I2R(Jv[wp + uP])/ I2R(kv)))
				set Wp = 1
				loop
					exitwhen Wp > yp
				...
				call MultiboardSetItemValueBJ(StatsBoard,10,(GB + uP + 3),I2S(ScoreArr[wp + uP]))
				set R = wp
			endif
			set wp = wp + 1
		endloop
